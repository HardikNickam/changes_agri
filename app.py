...
def crop_prediction():
    if request.method == "POST":
        ...
        predictions_proba = model.predict_proba(
            [[nitrogen, phosphorus, potassium, temperature, humidity, phLevel, rainfall]])
        ...
        top_crop_statement = "The top 5 predicted crops are: " + ", ".join(
            [crop.capitalize() for crop, _ in top_results])
        ...
        for crop, _ in top_results:
            new_prediction = PredictionResult(user_id=user.id, crop_name=crop,
                                              predicted_price=_)  # Replace '_' with the actual predicted price
            db.session.add(new_prediction)
        db.session.commit()

        return render_template('crop_result.html', top_results=top_results, prediction_text=top_crop_statement,
                               crop_images=predicted_crops, prediction=true_label, image_path=image_path)
